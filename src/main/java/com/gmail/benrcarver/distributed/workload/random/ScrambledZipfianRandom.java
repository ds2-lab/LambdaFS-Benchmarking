 ...
 */
public class ScrambledZipfianRandom extends NumberRandom {
   ...
   */
  public ScrambledZipfianRandom(long items) {
    this(0, items - 1);
  }
   ...
   */
  public ScrambledZipfianRandom(long min, long max) {
    this(min, max, ZipfianRandom.ZIPFIAN_CONSTANT);
  }
...
*/
   ...
   */
  public ScrambledZipfianRandom(long min, long max, double zipfianconstant) {
    this.min = min;
    this.max = max;
    itemcount = this.max - this.min + 1;
    if (zipfianconstant == USED_ZIPFIAN_CONSTANT) {
      gen = new ZipfianRandom(0, ITEM_COUNT, zipfianconstant, ZETAN);
    } else {
      gen = new ZipfianRandom(0, ITEM_COUNT, zipfianconstant);
    }
  }
  ...
  public static void main(String[] args) {
    double newzetan = ZipfianRandom.zetastatic(ITEM_COUNT, ZipfianRandom.ZIPFIAN_CONSTANT);
    System.out.println("zetan: " + newzetan);
    System.exit(0);

    ScrambledZipfianRandom gen = new ScrambledZipfianRandom(10000);
